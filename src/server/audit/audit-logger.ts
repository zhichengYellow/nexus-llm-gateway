...
export class AuditLogger {
  ...
  async query(options: {
    ...
    try {
      const { default: drizzle } = await import("drizzle-orm");
      const conditions: any[] = [];
      
      // 简单查询（不用动态 where builder）
      const rows = await db
        .select()
        .from(auditLogs)
  ...
  async count(): Promise<number> {
    try {
      const [{ default: drizzle }] = await Promise.all([import("drizzle-orm")]);
      const [row] = await db.select({ cnt: (await import("drizzle-orm")).sql<number>`count(*)::int` }).from(auditLogs);
